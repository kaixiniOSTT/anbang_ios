...
@implementation MWPhotoBrowser {
    UIActionSheet *_longPressedActionSheet;
}
...
- (void)_initialisation {
    ...
    // Listen for MWTapDetectingImageView notifications
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleMWImageViewLongPressNotification:)
                                                 name:MWImageViewLongPressNotification
                                               object:nil];
}
...
- (void)handleMWImageViewLongPressNotification:(NSNotification *)notification {
    if (!_longPressedActionSheet) {
        _longPressedActionSheet = [[UIActionSheet alloc] initWithTitle:nil
                                                              delegate:self
                                                     cancelButtonTitle:@"取消"
                                                destructiveButtonTitle:nil
                                                     otherButtonTitles:@"保存图片", nil];
    }
    [_longPressedActionSheet showInView:[[UIApplication sharedApplication] keyWindow]];
}

- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowser:clickedLongPressedActionSheetAtIndex:)]) {
        [self.delegate photoBrowser:self clickedLongPressedActionSheetAtIndex:buttonIndex];
    }
}
#pragma mark - Paging
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    if ([MFMailComposeViewController canSendMail]) {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               ...
                                                               otherButtonTitles:NSLocalizedString(@"保存", nil), NSLocalizedString(@"复制", nil), NSLocalizedString(@"邮件", nil), nil];
                    } else {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               cancelButtonTitle:NSLocalizedString(@"取消", nil) destructiveButtonTitle:nil
