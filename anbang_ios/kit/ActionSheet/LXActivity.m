...
#define SHAREBUTTON_INTERVAL_WIDTH               [UIScreen mainScreen].bounds.size.width/6.4
#define SHAREBUTTON_INTERVAL_HEIGHT             35

//#define SHARETITLE_WIDTH                        50
//silencesky upd
#define SHARETITLE_WIDTH                        [UIScreen mainScreen].bounds.size.width/6.4
        ...
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                if (self.isHadTitle == YES) {
                    ...
                    //silencesky upd
                    [shareLabel setFrame:CGRectMake(SHAREBUTTON_INTERVAL_WIDTH+((line-1)*(SHAREBUTTON_INTERVAL_WIDTH+SHAREBUTTON_WIDTH)), self.LXActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHAREBUTTON_WIDTH, SHARETITLE_HEIGHT)];
                }
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
           // [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
            //silencesky upd
             [cancelButton setFrame:CGRectMake((KCurrWidth-cancelButton.frame.size.width)/2, self.LXActivityHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
