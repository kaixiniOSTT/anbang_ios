...
- (void)sendOpeningNegotiation
{
    ...
    if ([self isP2P])
    {
		...
		{
			temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' from='%@' to='%@' lang='%@'>";
			s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream, [myJID_setByClient bare], [remoteJID bare],[[NSUserDefaults standardUserDefaults]objectForKey:@"NSUD_language"]];
		}
		else if (myJID_setByClient)
		{
			temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' from='%@' lang='%@'>";
			s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream, [myJID_setByClient bare],[[NSUserDefaults standardUserDefaults]objectForKey:@"NSUD_language"]];
		}
		else if (remoteJID)
		{
			temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' to='%@' lang='%@'>";
			s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream, [remoteJID bare],[[NSUserDefaults standardUserDefaults]objectForKey:@"NSUD_language"]];
		}
		else
		{
			temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' lang='%@'>";
			s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream,[[NSUserDefaults standardUserDefaults]objectForKey:@"NSUD_language"]];
		}
    }
    else
    {
		if (myJID_setByClient)
		{
            ...
            NSString *toStr = [NSString stringWithFormat:@"%@/ipa_%@",[myJID_setByClient domain],currentVersion];
            
            NSLog(@"domain:%@",[myJID_setByClient domain]);
            s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream, toStr,[[NSUserDefaults standardUserDefaults]objectForKey:@"NSUD_language"]];
		}
        else if ([hostName length] > 0)
        {
            temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' to='%@' lang='%@'>";
            s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream, hostName,[[NSUserDefaults standardUserDefaults]objectForKey:@"NSUD_language"]];
        }
        else
        {
            temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' lang='%@'>";
