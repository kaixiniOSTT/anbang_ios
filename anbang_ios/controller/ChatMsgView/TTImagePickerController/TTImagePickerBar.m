...
- (void)updateTipLabel
{
    ...
    self.tipLabel.text = [NSString stringWithFormat:@"已选择 %d 张，还可以选择 %d 张", count, self.maxSelected -count];
}
...
- (void)reloadData
{
    ...
    if (rect.origin.x > KCurrWidth) {
        [self setContentOffset:(CGPoint){rect.origin.x-KCurrWidth, 0} animated:YES];
    }
    
    [self updateTipLabel];
}

-(void)setMaxSelected:(NSInteger)maxSelected
{
