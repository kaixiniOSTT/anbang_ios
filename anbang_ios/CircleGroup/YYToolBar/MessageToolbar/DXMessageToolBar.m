...
@interface DXMessageToolBar()<UITextViewDelegate, DXFaceDelegate,ZBMessageManagerFaceViewDelegate>
{
...
- (ZBMessageManagerFaceView *)faceView{
    if (!_faceView){
        _faceView = [[ZBMessageManagerFaceView alloc]initWithFrame:CGRectMake(0.0f,
                                                                              CGRectGetHeight(self.frame), CGRectGetWidth(self.frame), 226)];
        
        //        _faceView.sectionBar.hidden = YES;
    ...
    if(dele){
        if(text != nil && ![text isEqualToString:@""]){
            ...
            if ([text isEqualToString:replaced]) {
                ...
                self.inputTextView.text = replaced;
            }
            return;
        }
    }
    
    self.inputTextView.text = [NSString stringWithFormat:@"%@%@",text,faceStr];
...
- (void)setupSubviews
{
    ...
    if ([_isToalAction isEqualToString:@"1"]){
        self.styleChangeButton.hidden = YES;
    }
    
    //发送
    self.sendButton = [[UIButton alloc] init];
    self.sendButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - kInputTextViewMinHeight, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight);
    if ([_isToalAction isEqualToString:@"1"]) {
        [self.sendButton setImage:[UIImage imageNamed:@"chat_button_send"] forState:UIControlStateNormal];
    }else{
        [self.sendButton setImage:[UIImage imageNamed:@"chatBar_more"] forState:UIControlStateNormal];
        [self.sendButton setImage:[UIImage imageNamed:@"chatBar_moreSelected"] forState:UIControlStateHighlighted];
        [self.sendButton setImage:[UIImage imageNamed:@"chatBar_keyboard"] forState:UIControlStateSelected];
    }
    ...
    self.faceButton.frame = CGRectMake(CGRectGetMinX(self.sendButton.frame) - kInputTextViewMinHeight - kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight);
    ...
    if ([_isToalAction isEqualToString:@"1"]) {
        self.inputTextView.frame = CGRectMake(10, kVerticalPadding, width+55, kInputTextViewMinHeight);
    }
    else{
    ...
    if ([_isToalAction isEqualToString:@"1"]) {
        self.recordButton.frame = CGRectMake(textViewLeftMargin, kVerticalPadding, width+15, kInputTextViewMinHeight);
    }
    else{
    ...
    if (!self.moreView) {
        self.moreView = [[DXChatBarMoreView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 100)];
...
- (void)recordButtonTouchUpInside
{
    ...
    if ([self.delegate respondsToSelector:@selector(didFinishRecoingVoiceAction:time:)])
    {
        if ([self.recordView isKindOfClass:[DXRecordView class]]) {
            DXRecordView * vedioView = (DXRecordView *)self.recordView;
            [self.delegate didFinishRecoingVoiceAction:vedioView.vedioUrl time:vedioView.interNum];
        }
    }
